...
const domain = paymentsDomain(demoMandate(scenarios));
...
const pre = await measure([new FunctionImplementation(before, reconcile, "simulation")], distribution, scenarios, { domain, config });
const post = await measure([new FunctionImplementation(after, slightlyDifferentPolicy, "simulation")], distribution, scenarios, { domain, config });
