...
| Constructed labels and hazards | `Trajectory.label` is any string fixed by the creator; payments `generators` inject eight hazard types | The label rule is written into the distribution; the SDK never constrains the vocabulary. |
...
| Paired clustered comparisons | Trajectory differences, top-cluster bootstrap | Independent exchangeable top clusters are an assumption. |
...
| Difficulty | Logistic mixed model, function-by-label, trajectory and trajectory/function effects | Approximate Bayesian model; leave-function-out refits; modest-book memory scale. |
| New-book prediction | New random effects and outcomes over 10,000 trajectories | Width check may fail on a book; failure is reported, not hidden. |
...
| Pre/post report | Same trajectories/seeds, bound plan, paired results and both certificates | Different IDs are called different functions. |
